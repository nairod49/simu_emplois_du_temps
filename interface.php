...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
			...
			<input type="radio" id='l1' name="formation" value="L1_MPCIE" checked />
			<label >L1 MPCIE</label>

			<input type="radio" id='l2' name="formation" value="L2_MPCIE"  />
			<label>L2 MPCIE</label>
			
			<input type="radio" id='l3i' name="formation"value="L3_INFO"  />
			<label>L3 INFO</label>
			
			<input type="radio" id='l3p' name="formation" value="L3_PRO"  />
			<label>L3 PRO</label>
			<br>
			...
			<?php if(!empty($_POST['donne_forma'])) {
	efface_tout();
	...
	<h3>Les Etudiants</h3>
		...
		<input type="number" id="nbeleve" name="nbeleve" value="50"  min="2" max="500" > <br><br>
			...
			//for($i=0;$i<count($list_formation[$_POST['formation']]);$i++){
			//	$nom=$list_formation[$_POST['formation']]->parcours;
			//	var_dump($nom);
...
base_matiere();
	...
	if(isset($_POST['envoyer'])) {
		...
		var_dump($_POST['formation']);
			creation_groupe_etudiant($_POST['nbeleve'],$list_formation[$_POST['formation']]);
		...
		creation_groupe($list_formation[$_POST['formation']],$_POST['nbtd'],$_POST['nbtp']);	
